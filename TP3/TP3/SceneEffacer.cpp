...
{
	if (!ecranMenuT.loadFromFile("Ressources\\Sprites\\backgroundMenu.jpg"))
	{
		return false;
	}
	...
	textboxErreur.insererTexte("Est vous sur?");
	for (int i = 0; i< NBBUTTONS; ++i)
	{
		buttons[i].init(480, 24, Vector2f(640, (i * 50) + 300), font);
		if (i == 0)
			...
			for (int i = 0; i < NBBUTTONS; ++i)
			{
				...
				if (buttons[i].touche(Mouse::getPosition(*mainWin)))
				{
					...
					if (i == 2)
					{
						...
						if (vue.lastDataEffacerState() == 0)
						{
							if (vue.confirmEffacerCompte(buttons[0].getTexte(), buttons[1].getTexte())) // Vérification du compte
						...
						else if (vue.lastDataEffacerState() == 1)
						{
							vue.effacerCompte(); // Effacer le compte
